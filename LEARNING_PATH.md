...
当前这些对象仍只是经过测试的协议值，尚未接入生产产品所需的持久 Thread/Message、长期 AgentInstance、独立 AgentSession、append-only Event Journal、durable queue、真实 Agent 讨论与收敛因果链、ContextManifest、硬取消和操作系统级隔离。协议存在不等于运行能力已经存在。
...
严格按 01A～01E 学习：01A 已实现 Scope/Thread/Turn/Message、通用 AgentProfile/Role、AgentSession、Invocation、Outcome/Acceptance 和 Event 协议；当前 01B 实现状态 Store、Journal 与 Outbox；01C 实现 durable Invocation；01D 接入现有 Task/Scenario/Coding 和 Web 查询；01E 完成 INC-01 并启动四组 INC-02 Shadow。
...
状态表是当前业务真相源，Journal 是不可变审计，Snapshot 是兼容恢复检查点；关键状态、Event 与 Outbox 同事务提交。PROD-01 的 Invocation 只冻结输入引用与摘要、策略快照引用和预算预留，不提前假装实现完整 ContextManifest 或 Tool Grant。
