...
`PRODUCT-01C-BACKEND-SESSION-BINDING`离线纵切已完成。`AgentExecutionRequest/Result`现在使用`backend_id`和`backend_session_id`区分Backend私有连续性与Harness的`agent_session_id`；Runtime SQLite schema升至v8，以`scope_id + thread_id + agent_id + backend_id`持久唯一绑定，且同一`backend_id + backend_session_id`不能绑定第二个Agent或Thread。第一次Fake调用返回的ID在Database/Store/Runtime重建后被自动注入同一Agent的新Invocation；调用方显式提交错误ID、跨Agent、跨Thread、跨Backend复用，或已完成结果切换Backend重放，均在Backend前以稳定错误拒绝。原始替换红测进一步证明`INSERT OR REPLACE`曾可覆盖绑定，新增append-only replace trigger后转绿。Codex Session事件和脱敏smoke不再公开具体ID，只保留`session_observed`布尔。定向54/54、Runtime 185/185、全仓非expected-red 635/635（9 skip）通过，真实CLI/模型/网络调用0。本切尚未证明真实Codex resume，也未实现Session缺失/失效后的权威Context重建；Session仍只是优化，不是任务真相。

`PRODUCT-01C-BACKEND-SESSION-CONTEXT-REBUILD`离线Runtime纵切已完成。SQLite schema升至v9，以不可变、digest绑定的Recovery Context保存当前Task Snapshot、Permission Snapshot、Message和Artifact正文/引用；正文SHA-256必须与权威State Envelope中的引用一致。只有Backend明确抛出typed `BackendSessionUnavailable`且失效ID等于当前私有绑定时，Runtime才丢弃旧ID、用持久上下文发起一次新Session调用；普通错误不恢复，缺少上下文直接`recovery_context_not_found`，第二次仍失效则`backend_session_recovery_failed`并停止。新Session按append-only generation保存，旧绑定和恢复链均不覆盖；重建Prompt不含旧/新Session ID。定向59/59、Runtime 185/185、全仓非expected-red 640/640（9 skip）通过，真实CLI/模型/网络调用0。该切片证明Runtime恢复合同，不表示Codex CLI Adapter已经能从真实CLI错误中可靠识别Session失效；下一切片必须先冻结并TDD实现这项错误分类，再申请真实resume/失效复验。

`PRODUCT-01C-CODEX-SESSION-ERROR-CLASSIFICATION`离线证据边界已完成。OpenAI官方非交互文档确认`--json`输出JSONL且可能包含`turn.failed`和`error`事件，也确认可按具体Session ID执行`codex exec resume`，但当前页面没有定义“指定Session不存在/失效”的稳定错误码或字段；本机`codex-cli 0.149.0-alpha.4.3`的resume help同样没有该合同。因此实现没有扫描stderr或自然语言消息，而是在`CodexCliProcessResult`增加受校验的公开`failure_kind`字符串：只有受信Transport明确给出`backend_session_unavailable`、当前确实是resume请求、进程非零退出且没有timeout时，Adapter才抛typed Runtime信号。超时与矛盾证据转成安全协议错误，stderr即使包含“Session not found”也只是普通失败；损坏JSONL转成不携带原文的`RuntimeProtocolError`。最初把Enum对象直接放入受批准结果时被现有capability防逃逸门正确拒绝，改为边界内归一化的普通字符串后恢复。Adapter→Runtime→SQLite Context重建链由Fake Transport转绿；定向64/64、Runtime 185/185、全仓非expected-red 645/645（9 skip）通过，真实Agent/模型/网络调用0。

用户随后授权且只执行了一次受控无效Session探针。探针使用固定不存在的测试UUID、read-only权限、10秒上限且不重试；当前CLI在1654ms内非零退出，stdout为0行，没有JSONL事件，也没有`thread.started`、`turn.started`、`item.*`或其他模型执行迹象，仅观察到stderr存在。原始stderr、Session ID、路径和认证信息均未读取或公开。这个结果证明“当前版本在该路径上不会提供可供Transport映射的结构化stdout证据”，但不能仅凭stderr存在把失败断言为Session失效。诊断入口及泄漏/误启动模型防护已由Fake转绿，定向66/66、Runtime 185/185、全仓非expected-red 647/647（9 skip）通过。真实Transport继续fail-closed；下一步必须由用户在“手动确认后新建Session”和“所有启动前resume失败自动新建一次”之间选择，不能静默把认证、配置或启动失败误判成Session失效。

用户选择并已完成`PRODUCT-01C-MANUAL-SESSION-RECOVERY-CONFIRMATION`离线纵切。Runtime不再在Session失效后自动调用第二次Backend：typed Session失效，或带当前私有Session的resume在产生任何公开事件前返回失败，都会持久进入`awaiting_user_confirmation`。公开Prompt只含schema、状态、确认ID、Invocation ID、固定说明和`create_new_session/stop_task`两个选择，不含Backend Session、Context正文、Workspace或stderr。SQLite schema升至v10，以append-only请求、决定和尝试三张表分别保存“等待什么、用户选了什么、这次确认是否已经领取过执行机会”；确认精确绑定当前State/Context、Thread、Agent和Backend。选择停止后返回可重放`stopped`且零Backend调用；选择继续后先一次性领取尝试，再以持久Recovery Context清空旧Session创建新Session。刷新等待页面不会重调Backend，重复点击成功确认会重放已持久结果；错误确认、改选和失败后的重复点击均在Backend前拒绝。若确认后的尝试失败或在结果提交前中断，后续运行返回`recovery_attempt_unresolved`，不会形成第三次调用或死循环。定向70/70、Runtime 185/185、全仓非expected-red 651/651（9 skip）通过，真实CLI/模型/网络调用0。当前完成的是Runtime与持久接口，不是FastAPI/Web按钮或真实恢复验收。

用户随后单独授权并完成`PRODUCT-01C-REAL-MANUAL-SESSION-RECOVERY-SMOKE`。新增的脱敏公开入口先用Fake经历RED→GREEN，再最多启动两个真实Codex进程：第一次固定无效resume没有产生任何公开事件，Runtime持久返回`awaiting_user_confirmation`；用户已确认继续后，第二次清空旧Session，从权威Recovery Context创建一个read-only新Session并完成。公开检查9/9为true：replacement Session被观察并持久，相同Invocation重放没有第三次调用，Agent回报Workspace未修改。总耗时9823 ms，第二次真实模型Usage为输入15528、缓存11008、输出14、reasoning输出0；公开报告不含新旧Session ID、confirmation ID、Context正文、SQLite/Workspace路径、stderr/stdout原文或认证信息。相关35/35、全仓非专用行为red-card集合652/652（9 skip）、py_compile和diff-check通过。这关闭了当前版本的真实人工Session恢复验收，不表示已完成通用CLI错误分类、取消或exactly-once。

`PRODUCT-01C-EXACT-CHANGESET-USER-APPROVAL`已完成离线TDD纵切。新增`ChangeApprovalRuntime.propose / approve / apply`公共门：ChangeSet把提案ID、Scope、受保护目标类型/引用、理由、精确变更对象、影响引用、请求权限、依赖digest、证据引用、风险、验证计划和base state digest全部纳入一个canonical `change_digest`。SQLite schema升至v11，proposal、user approval、application claim和applied receipt分成四类append-only事实；无批准、任意精确内容/目标/权限/依赖/证据/风险/验证说明变化、base state漂移和重复应用都在effect前拒绝。用户批准绑定精确proposal ID + change digest + base state digest，跨Runtime重建仍有效，但只能成功应用一次；effect失败或claim后崩溃进入`change_application_unresolved`，不自动重试且不宣称exactly-once。新门3/3、SQLite/恢复定向106/106、全仓合法集合655/655（9 skip）、py_compile和diff-check通过，真实模型和真实系统变更应用0。当前Agent Action没有user approval动作；后续只能由本地产品API的用户交互入口构造approval confirmation，该Web按钮尚未接入。

`PRODUCT-01C-LOCAL-PRODUCT-SERVICE-CHAIN`首个离线纵切已完成。新增公开`LocalProductTaskService.run(ProductTaskRequest) → ProductTaskResult`，把现有AgentExecutionRuntime、SQLite执行状态/结果、RoleAssignment、Message/Mailbox、Artifact和Runtime-owned Validator第一次装配成一条产品服务链。Planner首切只返回严格`planner-delegation/v1`单步委派，只能指定Role、任务说明、所需能力和验收摘要，不能指定具体Agent；Runtime确定性选择接收Agent并以同事务保存Assignment与Mailbox消息，领取后才发起接收Agent Invocation。接收Agent最终文本只形成未验证候选Artifact，独立Validator提供证据后才能返回`validated`。公共红测先因产品服务模块不存在而ImportError，最小实现后Planner与Reviewer Fake Executor各恰好调用一次；邻近49/49、全仓合法集合656/656（9 skip）、py_compile和diff-check通过，真实CLI/模型/网络调用0。当前Artifact/Verification仍使用现有内存Store，尚不能跨进程重开；畸形Planner输出、Validator失败/unknown、服务重放和跨进程结果历史是紧邻子切，完成前不把该链路称为本地API或可重开产品历史。

`PRODUCT-01C-DURABLE-PRODUCT-ACCEPTANCE-HISTORY`已完成首个validated纵切。Runtime SQLite schema升至v12，以同一事务append-only保存公开ProductTaskResult receipt、本任务候选/Validator evidence/report Artifacts和最终VerificationRecord；每条JSON使用canonical编码与SHA-256，task ID或引用碰撞不能覆盖旧验收。`LocalProductTaskService.get_task_result/get_artifact/get_verification`成为公开历史读取入口。公共红测先证明重建Database与Service后没有读取方法；最小实现后使用全新内存ArtifactStore与零调用Fake Executor，仍能读回与首次完全相同的结果、候选正文、Validator报告和passed Verification。新服务测试2/2、SQLite/Runtime定向117/117、全仓657/657（9 skip）、py_compile和diff-check通过，真实CLI/模型/网络调用0。持久内容仅含公开结果、Artifact、证据和验证记录，不含Prompt、Backend Session或stderr。
